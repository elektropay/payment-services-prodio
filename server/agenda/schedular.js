...
module.exports = {
    initAgenda : (DB_NAME)=>{
        ( async function() {
            ...
            agenda = new Agenda({db: {address: mongoConnectionString,collection:'agendaJobs'},useNewUrlParser: true});

            await agenda.start();
            setTimeout(function(){syncAllJobs(agenda,mongoConnectionString,DB_NAME);},3000);
    ...
    rescheduleEvent: (jobId,jobData) =>{
        ...
        agenda.jobs({name: jobId}, (error, jobs) => {
          const job = jobs[0];
